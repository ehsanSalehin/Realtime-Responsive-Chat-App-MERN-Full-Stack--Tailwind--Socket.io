    ...
    return (
      <div className={`${isSender ? "text-right" : "text-left"}`}>
        {message.messageType === "text" && (
          <div
            className={`${
              isSender ? "bg-[#26a1c3] text-white border-white/20" : "bg-[#9785ff] text-white/90 border-white/20"
    ...
    return (
      <div className={`mt-5 ${isSender ? "text-right" : "text-left"}`}>
        {message.messageType === "text" && (
          <div
            className={`${
              isSender ? "bg-[#9785ff] text-white/90" : "bg-[#26a1c3] text-white"
            } border-white/20 border inline-block p-4 rounded my-1 max-w-[50%] break-words`}
          >
            {message.content}
          </div>
        )}
    ...
    <div className="flex-1 overflow-y-auto scrollbar-hidden p-4 px-8 md:w-[65vw] lg:w-[78vw] xl-[88vw]">
      ...
      {showImage && (
        <div className="fixed z-[1000] top-0 left-0 h-[100vh] w-[100vw] flex items-center justify-center flex-col backdrop-blur-lg bg-black/70">
          ...
          />
          <span
            className="bg-white/20 p-3 text-2xl rounded-full hover:bg-white/50 cursor-pointer transition-all duration-100 mt-2"
